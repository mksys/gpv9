class FuelController < ApplicationController
  ...
  def index
    @site_section = "Encyclopedia: Fuel"
    @site_section_number = 8
 
    @stories = Story.live_date.published.alpha_headline_order.gpe_fuel_category.all 
  end
  ...
  def show
    ...
    def showlegacy
      ...
      @site_section_number = 8
        ...
        if Story.find_by_legacy_filename("gpe/fue-" + params[:story] + ".html" )
           @story = Story.find_by_legacy_filename("gpe/fue-" + params[:story] + ".html" )

           if @story.category_id == 9 # make sure it is a fuel story
             respond_to do |format|
